...
std::optional<T> PythonBinding<T>::from_python(BorrowedPyObject *obj) noexcept {
  ...
  if (tp < &(gTypes[626]) || tp >= &(gTypes[627])) {
    return std::nullopt;
  }
...
PyTypeObject *InitType(void) noexcept {
  PyTypeObject * const tp = &(gTypes[626]);
  ...
  tp->tp_hash = PythonBinding<mx::AbstractConditionalOperator>::type()->tp_hash;
  tp->tp_richcompare = nullptr;
  tp->tp_iter = nullptr;  // TODO
  tp->tp_methods = gMethods;
  tp->tp_getset = gProperties;
  tp->tp_base = PythonBinding<mx::AbstractConditionalOperator>::type();
