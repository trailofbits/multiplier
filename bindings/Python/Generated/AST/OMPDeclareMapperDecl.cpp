...
std::optional<T> PythonBinding<T>::from_python(BorrowedPyObject *obj) noexcept {
  ...
  if (tp < &(gTypes[748]) || tp >= &(gTypes[749])) {
    return std::nullopt;
  }
...
PyTypeObject *InitType(void) noexcept {
  ...
  tp->tp_hash = PythonBinding<mx::OMPDeclarativeDirectiveValueDecl>::type()->tp_hash;
  tp->tp_richcompare = nullptr;
  tp->tp_iter = nullptr;
  tp->tp_methods = gMethods;
  tp->tp_getset = gProperties;
  tp->tp_base = PythonBinding<mx::OMPDeclarativeDirectiveValueDecl>::type();
