...
std::optional<T> PythonBinding<T>::from_python(BorrowedPyObject *obj) noexcept {
  ...
  if (tp < &(gTypes[389]) || tp >= &(gTypes[390])) {
    return std::nullopt;
  }
...
static PyGetSetDef gProperties[] = {
  {
    ...
    nullptr,
    PyDoc_STR("Wrapper for mx::RecordType::desugar"),
    nullptr,
  },
  {
    "has_const_fields",
...
PyTypeObject *InitType(void) noexcept {
  PyTypeObject * const tp = &(gTypes[389]);
  ...
  tp->tp_hash = gTypes[388].tp_hash;
  tp->tp_richcompare = gTypes[388].tp_richcompare;
  tp->tp_iter = nullptr;
  tp->tp_methods = gMethods;
  tp->tp_getset = gProperties;
  tp->tp_base = &(gTypes[388]);
