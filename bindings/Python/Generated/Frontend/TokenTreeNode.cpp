...
std::optional<T> PythonBinding<T>::from_python(BorrowedPyObject *obj) noexcept {
  ...
  if (tp < &(gTypes[861]) || tp >= &(gTypes[867])) {
    return std::nullopt;
  }
...
bool PythonBinding<T>::load(BorrowedPyObject *module) noexcept {
  ...
  if (0 != PyModule_AddObjectRef(module, "TokenTreeNode", tp_obj)) {
    return false;
  }
...
static PyGetSetDef gProperties[] = {
  {
    ...
    nullptr,
    PyDoc_STR("Wrapper for mx::TokenTreeNode::kind"),
    nullptr,
  },
  {}  // Sentinel.
};
...
PyTypeObject *InitType(void) noexcept {
  ...
  tp->tp_name = "multiplier.frontend.TokenTreeNode";
  tp->tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_DISALLOW_INSTANTIATION;
  tp->tp_doc = PyDoc_STR("Wrapper for mx::::TokenTreeNode");
  ...
  tp->tp_init = [] (BorrowedPyObject *self, BorrowedPyObject *args, BorrowedPyObject *kwargs) -> int {
    if (kwargs && (!PyMapping_Check(kwargs) || PyMapping_Size(kwargs))) {
      PyErrorStreamer(PyExc_TypeError)
          << "'TokenTreeNode.__init__' does not take any keyword arguments";
      return -1;
    }

    if (!args || !PySequence_Check(args)) {
      PyErrorStreamer(PyExc_TypeError)
          << "Invalid positional arguments passed to 'TokenTreeNode.__init__'";
      return -1;
    }
    ...
    (void) num_args;
    PyErrorStreamer(PyExc_TypeError)
        << "Class 'TokenTreeNode' cannot be directly instantiated";
    return -1;
