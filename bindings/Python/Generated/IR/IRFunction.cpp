...
bool PythonBinding<T>::load(BorrowedPyObject *module) noexcept {
  ...
  if (0 != PyModule_AddObjectRef(module, "IRFunction", tp_obj)) {
    return false;
  }
...
PyTypeObject *InitType(void) noexcept {
  ...
  tp->tp_name = "multiplier.ir.IRFunction";
  tp->tp_flags = Py_TPFLAGS_DEFAULT;
  tp->tp_doc = PyDoc_STR("Wrapper for mx::::IRFunction");
  ...
  tp->tp_init = [] (BorrowedPyObject *self, BorrowedPyObject *args, BorrowedPyObject *kwargs) -> int {
    if (kwargs && (!PyMapping_Check(kwargs) || PyMapping_Size(kwargs))) {
      PyErrorStreamer(PyExc_TypeError)
          << "'IRFunction.__init__' does not take any keyword arguments";
      return -1;
    }

    if (!args || !PySequence_Check(args)) {
      PyErrorStreamer(PyExc_TypeError)
          << "Invalid positional arguments passed to 'IRFunction.__init__'";
      return -1;
    }
    ...
    while (num_args == 0) {
      obj->data = new (obj->backing_storage) IRFunction();
      return 0;
    }

    PyErrorStreamer(PyExc_TypeError)
        << "Invalid arguments to 'IRFunction.__init__'";
    return -1;
