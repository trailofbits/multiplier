...
std::optional<T> PythonBinding<T>::from_python(BorrowedPyObject *obj) noexcept {
  ...
  if (tp < &(gTypes[1361]) || tp >= &(gTypes[1364])) {
    return std::nullopt;
  }
...
static PyMethodDef gMethods[] = {
  {
    ...
    PyDoc_STR("Wrapper for mx::ir::unsup::Operation::from"),
  },
  {}  // Sentinel.
};
}  // namespace
...
PyTypeObject *InitType(void) noexcept {
  ...
  tp->tp_name = "multiplier.ir.unsupported.Operation";
  tp->tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_DISALLOW_INSTANTIATION;
  tp->tp_doc = PyDoc_STR("Wrapper for mx::ir::unsup::::Operation");
