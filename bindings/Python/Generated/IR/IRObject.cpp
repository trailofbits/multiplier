...
bool PythonBinding<T>::load(BorrowedPyObject *module) noexcept {
  ...
  if (0 != PyModule_AddObjectRef(module, "IRObject", tp_obj)) {
    return false;
  }
  ...
  tp->tp_init = [] (BorrowedPyObject *self, BorrowedPyObject *args, BorrowedPyObject *kwargs) -> int {
    if (kwargs && (!PyMapping_Check(kwargs) || PyMapping_Size(kwargs))) {
      PyErrorStreamer(PyExc_TypeError)
          << "'IRObject.__init__' does not take any keyword arguments";
      return -1;
    }

    if (!args || !PySequence_Check(args)) {
      PyErrorStreamer(PyExc_TypeError)
          << "Invalid positional arguments passed to 'IRObject.__init__'";
      return -1;
    }
    ...
    while (num_args == 0) {
      obj->data = new (obj->backing_storage) IRObject();
      return 0;
    }

    PyErrorStreamer(PyExc_TypeError)
        << "Invalid arguments to 'IRObject.__init__'";
    return -1;
