...
std::optional<StaticAssertDecl> StaticAssertDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<StaticAssertDecl, ir::hl::Operation>> StaticAssertDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kStaticAssertDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<StaticAssertDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
