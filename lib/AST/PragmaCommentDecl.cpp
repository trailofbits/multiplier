...
gap::generator<PragmaCommentDecl> PragmaCommentDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kPragmaCommentDeclDerivedKinds)) {
...
PragmaMSCommentKind PragmaCommentDecl::comment_kind(void) const {
  return static_cast<PragmaMSCommentKind>(impl->reader.getVal57());
}
