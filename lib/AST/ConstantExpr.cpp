...
std::optional<ConstantExpr> ConstantExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ConstantExpr, ir::hl::Operation>> ConstantExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kConstantExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ConstantExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
