...
std::optional<SEHTryStmt> SEHTryStmt::from(const ir::Operation &op) {
...
gap::generator<std::pair<SEHTryStmt, ir::Operation>> SEHTryStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kSEHTryStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SEHTryStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
