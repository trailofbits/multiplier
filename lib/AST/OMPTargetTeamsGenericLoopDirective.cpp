...
std::optional<OMPTargetTeamsGenericLoopDirective> OMPTargetTeamsGenericLoopDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPTargetTeamsGenericLoopDirective, ir::Operation>> OMPTargetTeamsGenericLoopDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPTargetTeamsGenericLoopDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTargetTeamsGenericLoopDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
