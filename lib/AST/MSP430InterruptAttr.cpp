...
gap::generator<MSP430InterruptAttr> MSP430InterruptAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMSP430InterruptAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MSP430InterruptAttr> e = from_base(std::move(eptr))) {
...
gap::generator<MSP430InterruptAttr> MSP430InterruptAttr::in(const File &file) {
...
std::optional<MSP430InterruptAttr> MSP430InterruptAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MSP430InterruptAttr> MSP430InterruptAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<MSP430InterruptAttr> MSP430InterruptAttr::from(const VariantEntity &e) {
