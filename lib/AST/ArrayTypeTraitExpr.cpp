...
std::optional<ArrayTypeTraitExpr> ArrayTypeTraitExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ArrayTypeTraitExpr, ir::hl::Operation>> ArrayTypeTraitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kArrayTypeTraitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ArrayTypeTraitExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
