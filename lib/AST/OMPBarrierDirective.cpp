...
gap::generator<std::pair<OMPBarrierDirective, ir::Operation>> OMPBarrierDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPBarrierDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPBarrierDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
