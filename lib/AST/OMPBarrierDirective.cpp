...
std::optional<OMPBarrierDirective> OMPBarrierDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPBarrierDirective, ir::hl::Operation>> OMPBarrierDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPBarrierDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPBarrierDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
