...
std::optional<GenericSelectionExpr> GenericSelectionExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<GenericSelectionExpr, ir::hl::Operation>> GenericSelectionExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kGenericSelectionExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<GenericSelectionExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
