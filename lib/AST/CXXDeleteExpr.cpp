...
std::optional<CXXDeleteExpr> CXXDeleteExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXDeleteExpr, ir::hl::Operation>> CXXDeleteExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXDeleteExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXDeleteExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
