...
std::optional<OMPSimdDirective> OMPSimdDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPSimdDirective, ir::Operation>> OMPSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPSimdDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
