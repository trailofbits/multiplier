...
gap::generator<TestTypestateAttr> TestTypestateAttr::in(const Index &index) {
  ...
  for (AttrKind k : kTestTypestateAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<TestTypestateAttr> e = from_base(std::move(eptr))) {
...
gap::generator<TestTypestateAttr> TestTypestateAttr::in(const File &file) {
...
std::optional<TestTypestateAttr> TestTypestateAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<TestTypestateAttr> TestTypestateAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<TestTypestateAttr> TestTypestateAttr::from(const VariantEntity &e) {
