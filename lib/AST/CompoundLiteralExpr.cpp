...
std::optional<CompoundLiteralExpr> CompoundLiteralExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CompoundLiteralExpr, ir::hl::Operation>> CompoundLiteralExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCompoundLiteralExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CompoundLiteralExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
