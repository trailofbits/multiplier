...
std::optional<OMPCancellationPointDirective> OMPCancellationPointDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPCancellationPointDirective, ir::hl::Operation>> OMPCancellationPointDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPCancellationPointDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPCancellationPointDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
