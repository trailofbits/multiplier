...
gap::generator<VisibilityAttr> VisibilityAttr::in(const Index &index) {
  ...
  for (AttrKind k : kVisibilityAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<VisibilityAttr> e = from_base(std::move(eptr))) {
...
std::optional<VisibilityAttr> VisibilityAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<VisibilityAttr> VisibilityAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<VisibilityAttr> VisibilityAttr::from(const VariantEntity &e) {
...
VisibilityAttrVisibilityType VisibilityAttr::visibility(void) const {
  return static_cast<VisibilityAttrVisibilityType>(impl->reader.getVal13());
}
