...
gap::generator<EnforceTCBAttr> EnforceTCBAttr::in(const Index &index) {
  ...
  for (AttrKind k : kEnforceTCBAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<EnforceTCBAttr> e = from_base(std::move(eptr))) {
...
std::optional<EnforceTCBAttr> EnforceTCBAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<EnforceTCBAttr> EnforceTCBAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<EnforceTCBAttr> EnforceTCBAttr::from(const VariantEntity &e) {
...
uint32_t EnforceTCBAttr::tcb_name_length(void) const {
