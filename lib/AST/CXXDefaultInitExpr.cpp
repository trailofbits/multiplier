...
std::optional<CXXDefaultInitExpr> CXXDefaultInitExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXDefaultInitExpr, ir::hl::Operation>> CXXDefaultInitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXDefaultInitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXDefaultInitExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
