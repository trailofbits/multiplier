...
std::optional<DeclRefExpr> DeclRefExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<DeclRefExpr, ir::hl::Operation>> DeclRefExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kDeclRefExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DeclRefExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
