...
gap::generator<FastCallAttr> FastCallAttr::in(const Index &index) {
  ...
  for (AttrKind k : kFastCallAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<FastCallAttr> e = from_base(std::move(eptr))) {
...
std::optional<FastCallAttr> FastCallAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<FastCallAttr> FastCallAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<FastCallAttr> FastCallAttr::from(const VariantEntity &e) {
