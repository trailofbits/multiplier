...
std::optional<CXXForRangeStmt> CXXForRangeStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXForRangeStmt, ir::hl::Operation>> CXXForRangeStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXForRangeStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXForRangeStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
