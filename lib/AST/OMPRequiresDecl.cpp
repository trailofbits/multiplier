...
std::optional<OMPRequiresDecl> OMPRequiresDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPRequiresDecl, ir::hl::Operation>> OMPRequiresDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kOMPRequiresDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPRequiresDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
