...
std::optional<TrivialABIAttr> TrivialABIAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<TrivialABIAttr> TrivialABIAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<TrivialABIAttr> TrivialABIAttr::in(const Index &index) {
  ...
  for (AttrKind k : kTrivialABIAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<TrivialABIAttr> e = from_base(std::move(eptr))) {
...
gap::generator<TrivialABIAttr> TrivialABIAttr::in(const File &file) {
...
std::optional<TrivialABIAttr> TrivialABIAttr::from(const VariantEntity &e) {
