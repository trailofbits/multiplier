...
std::optional<UsingEnumDecl> UsingEnumDecl::from(const ir::Operation &op) {
...
gap::generator<std::pair<UsingEnumDecl, ir::Operation>> UsingEnumDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kUsingEnumDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<UsingEnumDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
