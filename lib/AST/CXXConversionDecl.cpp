...
std::optional<CXXConversionDecl> CXXConversionDecl::from(const ir::Operation &op) {
...
gap::generator<std::pair<CXXConversionDecl, ir::Operation>> CXXConversionDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kCXXConversionDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXConversionDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
