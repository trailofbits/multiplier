...
std::optional<ObjCIvarRefExpr> ObjCIvarRefExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ObjCIvarRefExpr, ir::hl::Operation>> ObjCIvarRefExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kObjCIvarRefExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCIvarRefExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
