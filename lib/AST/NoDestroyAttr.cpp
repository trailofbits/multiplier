...
std::optional<NoDestroyAttr> NoDestroyAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<NoDestroyAttr> NoDestroyAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<NoDestroyAttr> NoDestroyAttr::in(const Index &index) {
  ...
  for (AttrKind k : kNoDestroyAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<NoDestroyAttr> e = from_base(std::move(eptr))) {
...
std::optional<NoDestroyAttr> NoDestroyAttr::from(const VariantEntity &e) {
