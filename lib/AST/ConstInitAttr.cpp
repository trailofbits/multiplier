...
std::optional<ConstInitAttr> ConstInitAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ConstInitAttr> ConstInitAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<ConstInitAttr> ConstInitAttr::in(const Index &index) {
  ...
  for (AttrKind k : kConstInitAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ConstInitAttr> e = from_base(std::move(eptr))) {
...
std::optional<ConstInitAttr> ConstInitAttr::from(const VariantEntity &e) {
