...
std::optional<StmtExpr> StmtExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<StmtExpr, ir::hl::Operation>> StmtExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kStmtExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<StmtExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
