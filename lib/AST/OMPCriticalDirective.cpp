...
std::optional<OMPCriticalDirective> OMPCriticalDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPCriticalDirective, ir::hl::Operation>> OMPCriticalDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPCriticalDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPCriticalDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
