...
gap::generator<WorkGroupSizeHintAttr> WorkGroupSizeHintAttr::in(const Index &index) {
  ...
  for (AttrKind k : kWorkGroupSizeHintAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<WorkGroupSizeHintAttr> e = from_base(std::move(eptr))) {
...
std::optional<WorkGroupSizeHintAttr> WorkGroupSizeHintAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<WorkGroupSizeHintAttr> WorkGroupSizeHintAttr::from(const Reference &r) {
  return WorkGroupSizeHintAttr::from(r.as_attribute());
}

std::optional<WorkGroupSizeHintAttr> WorkGroupSizeHintAttr::from(const VariantEntity &e) {
...
std::optional<WorkGroupSizeHintAttr> WorkGroupSizeHintAttr::from(const TokenContext &t) {
...
uint32_t WorkGroupSizeHintAttr::x_dim(void) const {
  return impl->reader.getVal12();
}

uint32_t WorkGroupSizeHintAttr::y_dim(void) const {
  return impl->reader.getVal25();
}

uint32_t WorkGroupSizeHintAttr::z_dim(void) const {
