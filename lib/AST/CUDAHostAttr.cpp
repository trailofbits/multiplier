...
std::optional<CUDAHostAttr> CUDAHostAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<CUDAHostAttr> CUDAHostAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<CUDAHostAttr> CUDAHostAttr::in(const Index &index) {
  ...
  for (AttrKind k : kCUDAHostAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<CUDAHostAttr> e = from_base(std::move(eptr))) {
...
std::optional<CUDAHostAttr> CUDAHostAttr::from(const VariantEntity &e) {
