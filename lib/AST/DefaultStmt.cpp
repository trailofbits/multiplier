...
std::optional<DefaultStmt> DefaultStmt::from(const ir::Operation &op) {
...
gap::generator<std::pair<DefaultStmt, ir::Operation>> DefaultStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kDefaultStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DefaultStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
