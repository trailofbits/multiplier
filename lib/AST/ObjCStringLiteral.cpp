...
std::optional<ObjCStringLiteral> ObjCStringLiteral::from(const ir::Operation &op) {
...
gap::generator<std::pair<ObjCStringLiteral, ir::Operation>> ObjCStringLiteral::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kObjCStringLiteralDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCStringLiteral, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
