...
std::optional<OMPCapturedExprDecl> OMPCapturedExprDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPCapturedExprDecl, ir::hl::Operation>> OMPCapturedExprDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kOMPCapturedExprDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPCapturedExprDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
