...
gap::generator<SysVABIAttr> SysVABIAttr::in(const Index &index) {
  ...
  for (AttrKind k : kSysVABIAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<SysVABIAttr> e = from_base(std::move(eptr))) {
...
std::optional<SysVABIAttr> SysVABIAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<SysVABIAttr> SysVABIAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<SysVABIAttr> SysVABIAttr::from(const VariantEntity &e) {
