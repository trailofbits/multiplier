...
std::optional<SubstNonTypeTemplateParmPackExpr> SubstNonTypeTemplateParmPackExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<SubstNonTypeTemplateParmPackExpr, ir::Operation>> SubstNonTypeTemplateParmPackExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kSubstNonTypeTemplateParmPackExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SubstNonTypeTemplateParmPackExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
