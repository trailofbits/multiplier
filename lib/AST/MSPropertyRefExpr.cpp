...
std::optional<MSPropertyRefExpr> MSPropertyRefExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<MSPropertyRefExpr, ir::hl::Operation>> MSPropertyRefExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kMSPropertyRefExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<MSPropertyRefExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
