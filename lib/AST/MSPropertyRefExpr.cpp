...
gap::generator<std::pair<MSPropertyRefExpr, ir::Operation>> MSPropertyRefExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kMSPropertyRefExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<MSPropertyRefExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
