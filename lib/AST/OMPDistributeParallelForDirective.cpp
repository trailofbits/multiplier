...
std::optional<OMPDistributeParallelForDirective> OMPDistributeParallelForDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPDistributeParallelForDirective, ir::hl::Operation>> OMPDistributeParallelForDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPDistributeParallelForDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPDistributeParallelForDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
