...
std::optional<ConvergentAttr> ConvergentAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ConvergentAttr> ConvergentAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<ConvergentAttr> ConvergentAttr::in(const Index &index) {
  ...
  for (AttrKind k : kConvergentAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ConvergentAttr> e = from_base(std::move(eptr))) {
...
std::optional<ConvergentAttr> ConvergentAttr::from(const VariantEntity &e) {
