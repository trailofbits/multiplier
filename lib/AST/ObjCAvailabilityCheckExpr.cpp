...
std::optional<ObjCAvailabilityCheckExpr> ObjCAvailabilityCheckExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<ObjCAvailabilityCheckExpr, ir::Operation>> ObjCAvailabilityCheckExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kObjCAvailabilityCheckExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCAvailabilityCheckExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
