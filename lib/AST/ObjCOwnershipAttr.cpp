...
gap::generator<ObjCOwnershipAttr> ObjCOwnershipAttr::in(const Index &index) {
  ...
  for (AttrKind k : kObjCOwnershipAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ObjCOwnershipAttr> e = from_base(std::move(eptr))) {
...
std::optional<ObjCOwnershipAttr> ObjCOwnershipAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ObjCOwnershipAttr> ObjCOwnershipAttr::from(const VariantEntity &e) {
