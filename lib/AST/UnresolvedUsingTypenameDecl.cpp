...
std::optional<UnresolvedUsingTypenameDecl> UnresolvedUsingTypenameDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<UnresolvedUsingTypenameDecl, ir::hl::Operation>> UnresolvedUsingTypenameDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kUnresolvedUsingTypenameDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<UnresolvedUsingTypenameDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
