...
std::optional<TemplateTypeParmDecl> TemplateTypeParmDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<TemplateTypeParmDecl, ir::hl::Operation>> TemplateTypeParmDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kTemplateTypeParmDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<TemplateTypeParmDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
