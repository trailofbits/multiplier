...
std::optional<OMPTaskLoopSimdDirective> OMPTaskLoopSimdDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPTaskLoopSimdDirective, ir::Operation>> OMPTaskLoopSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPTaskLoopSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTaskLoopSimdDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
