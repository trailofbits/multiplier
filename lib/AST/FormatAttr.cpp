...
std::optional<FormatAttr> FormatAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<FormatAttr> FormatAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<FormatAttr> FormatAttr::in(const Index &index) {
  ...
  for (AttrKind k : kFormatAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<FormatAttr> e = from_base(std::move(eptr))) {
...
gap::generator<FormatAttr> FormatAttr::in(const File &file) {
...
std::optional<FormatAttr> FormatAttr::from(const Reference &r) {
  return FormatAttr::from(r.as_attribute());
}

std::optional<FormatAttr> FormatAttr::from(const VariantEntity &e) {
...
std::optional<FormatAttr> FormatAttr::from(const TokenContext &t) {
