...
std::optional<CapturedStmt> CapturedStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CapturedStmt, ir::hl::Operation>> CapturedStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCapturedStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CapturedStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
