...
gap::generator<CapturedStmt> CapturedStmt::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kCapturedStmtDerivedKinds)) {
...
CapturedRegionKind CapturedStmt::captured_region_kind(void) const {
  return static_cast<CapturedRegionKind>(impl->reader.getVal58());
}
