...
std::optional<ObjCIsaExpr> ObjCIsaExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ObjCIsaExpr, ir::hl::Operation>> ObjCIsaExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kObjCIsaExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCIsaExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
