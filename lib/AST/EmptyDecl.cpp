...
std::optional<EmptyDecl> EmptyDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<EmptyDecl, ir::hl::Operation>> EmptyDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kEmptyDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<EmptyDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
