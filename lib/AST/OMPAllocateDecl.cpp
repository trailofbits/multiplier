...
std::optional<OMPAllocateDecl> OMPAllocateDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPAllocateDecl, ir::hl::Operation>> OMPAllocateDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kOMPAllocateDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPAllocateDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
