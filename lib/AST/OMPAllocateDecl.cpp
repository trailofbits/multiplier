...
std::optional<OMPAllocateDecl> OMPAllocateDecl::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPAllocateDecl, ir::Operation>> OMPAllocateDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kOMPAllocateDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPAllocateDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
gap::generator<Expr> OMPAllocateDecl::varlists(void) const & {
  ...
  for (auto v : list) {
    EntityId id(v);
    if (auto d50 = ep->StmtFor(ep, v)) {
      if (auto e = Expr::from_base(std::move(d50))) {
        co_yield std::move(*e);
      }
    }
  }
