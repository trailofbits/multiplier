...
gap::generator<TypeNonNullAttr> TypeNonNullAttr::in(const Index &index) {
  ...
  for (AttrKind k : kTypeNonNullAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<TypeNonNullAttr> e = from_base(std::move(eptr))) {
...
gap::generator<TypeNonNullAttr> TypeNonNullAttr::in(const File &file) {
...
std::optional<TypeNonNullAttr> TypeNonNullAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<TypeNonNullAttr> TypeNonNullAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<TypeNonNullAttr> TypeNonNullAttr::from(const Reference &r) {
  return TypeNonNullAttr::from(r.as_attribute());
}

std::optional<TypeNonNullAttr> TypeNonNullAttr::from(const VariantEntity &e) {
...
std::optional<TypeNonNullAttr> TypeNonNullAttr::from(const TokenContext &t) {
