...
std::optional<NonNullAttr> NonNullAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
gap::generator<NonNullAttr> NonNullAttr::in(const Index &index) {
  ...
  for (AttrKind k : kNonNullAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<NonNullAttr> e = from_base(std::move(eptr))) {
...
gap::generator<NonNullAttr> NonNullAttr::in(const File &file) {
...
std::optional<NonNullAttr> NonNullAttr::from(const VariantEntity &e) {
