...
std::optional<CXXThisExpr> CXXThisExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<CXXThisExpr, ir::Operation>> CXXThisExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kCXXThisExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXThisExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
