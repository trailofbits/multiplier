...
std::optional<DesignatedInitUpdateExpr> DesignatedInitUpdateExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<DesignatedInitUpdateExpr, ir::hl::Operation>> DesignatedInitUpdateExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kDesignatedInitUpdateExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DesignatedInitUpdateExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
