...
std::optional<IBOutletAttr> IBOutletAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<IBOutletAttr> IBOutletAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<IBOutletAttr> IBOutletAttr::in(const Index &index) {
  ...
  for (AttrKind k : kIBOutletAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<IBOutletAttr> e = from_base(std::move(eptr))) {
...
std::optional<IBOutletAttr> IBOutletAttr::from(const VariantEntity &e) {
