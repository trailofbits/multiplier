...
gap::generator<UnaryOperator> UnaryOperator::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kUnaryOperatorDerivedKinds)) {
...
UnaryOperatorKind UnaryOperator::opcode(void) const {
  return static_cast<UnaryOperatorKind>(impl->reader.getVal89());
}
