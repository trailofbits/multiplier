...
std::optional<IntegerLiteral> IntegerLiteral::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<IntegerLiteral, ir::hl::Operation>> IntegerLiteral::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kIntegerLiteralDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<IntegerLiteral, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
