...
std::optional<ObjCTypeParamDecl> ObjCTypeParamDecl::from(const ir::Operation &op) {
...
gap::generator<std::pair<ObjCTypeParamDecl, ir::Operation>> ObjCTypeParamDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kObjCTypeParamDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCTypeParamDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
ObjCTypeParamVariance ObjCTypeParamDecl::variance(void) const {
  return static_cast<ObjCTypeParamVariance>(impl->reader.getVal69());
}
