...
std::optional<SEHFinallyStmt> SEHFinallyStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<SEHFinallyStmt, ir::hl::Operation>> SEHFinallyStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kSEHFinallyStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SEHFinallyStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
