...
std::optional<WeakAttr> WeakAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<WeakAttr> WeakAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<WeakAttr> WeakAttr::in(const Index &index) {
  ...
  for (AttrKind k : kWeakAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<WeakAttr> e = from_base(std::move(eptr))) {
...
gap::generator<WeakAttr> WeakAttr::in(const File &file) {
...
std::optional<WeakAttr> WeakAttr::from(const VariantEntity &e) {
