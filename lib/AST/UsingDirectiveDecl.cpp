...
gap::generator<std::pair<UsingDirectiveDecl, ir::Operation>> UsingDirectiveDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kUsingDirectiveDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<UsingDirectiveDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
