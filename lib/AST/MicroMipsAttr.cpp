...
std::optional<MicroMipsAttr> MicroMipsAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MicroMipsAttr> MicroMipsAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<MicroMipsAttr> MicroMipsAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMicroMipsAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MicroMipsAttr> e = from_base(std::move(eptr))) {
...
gap::generator<MicroMipsAttr> MicroMipsAttr::in(const File &file) {
...
std::optional<MicroMipsAttr> MicroMipsAttr::from(const VariantEntity &e) {
