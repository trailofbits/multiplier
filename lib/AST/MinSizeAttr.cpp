...
std::optional<MinSizeAttr> MinSizeAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MinSizeAttr> MinSizeAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<MinSizeAttr> MinSizeAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMinSizeAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MinSizeAttr> e = from_base(std::move(eptr))) {
...
std::optional<MinSizeAttr> MinSizeAttr::from(const VariantEntity &e) {
