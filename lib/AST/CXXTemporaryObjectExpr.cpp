...
std::optional<CXXTemporaryObjectExpr> CXXTemporaryObjectExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<CXXTemporaryObjectExpr, ir::Operation>> CXXTemporaryObjectExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kCXXTemporaryObjectExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXTemporaryObjectExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
