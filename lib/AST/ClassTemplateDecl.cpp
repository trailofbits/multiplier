...
std::optional<ClassTemplateDecl> ClassTemplateDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ClassTemplateDecl, ir::hl::Operation>> ClassTemplateDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kClassTemplateDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ClassTemplateDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
gap::generator<ClassTemplateDecl> ClassTemplateDecl::redeclarations(void) const & {
  for (Decl r : Decl::redeclarations()) {
    if (std::optional<ClassTemplateDecl> dr = from_base(r)) {
...
std::optional<ClassTemplateDecl> ClassTemplateDecl::from(const std::optional<Decl> &parent) {
  if (parent) {
...
std::optional<ClassTemplateDecl> ClassTemplateDecl::from_base(const Decl &parent) {
  switch (parent.kind()) {
...
gap::generator<ClassTemplateDecl> ClassTemplateDecl::in(const Index &index) {
  ...
  for (DeclKind k : kClassTemplateDeclDerivedKinds) {
    for (DeclImplPtr eptr : ep->DeclsFor(ep, k)) {
      if (std::optional<ClassTemplateDecl> e = from_base(std::move(eptr))) {
...
std::optional<ClassTemplateDecl> ClassTemplateDecl::from(const VariantEntity &e) {
