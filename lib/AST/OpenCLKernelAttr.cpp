...
std::optional<OpenCLKernelAttr> OpenCLKernelAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
gap::generator<OpenCLKernelAttr> OpenCLKernelAttr::in(const Index &index) {
  ...
  for (AttrKind k : kOpenCLKernelAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<OpenCLKernelAttr> e = from_base(std::move(eptr))) {
...
std::optional<OpenCLKernelAttr> OpenCLKernelAttr::from(const VariantEntity &e) {
