...
std::optional<CoawaitExpr> CoawaitExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CoawaitExpr, ir::hl::Operation>> CoawaitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCoawaitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CoawaitExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
