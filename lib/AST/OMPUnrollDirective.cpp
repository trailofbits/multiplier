...
std::optional<OMPUnrollDirective> OMPUnrollDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPUnrollDirective, ir::Operation>> OMPUnrollDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPUnrollDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPUnrollDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
