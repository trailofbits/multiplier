...
std::optional<OMPUnrollDirective> OMPUnrollDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPUnrollDirective, ir::hl::Operation>> OMPUnrollDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPUnrollDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPUnrollDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
