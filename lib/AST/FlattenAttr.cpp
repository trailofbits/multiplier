...
gap::generator<FlattenAttr> FlattenAttr::in(const Index &index) {
  ...
  for (AttrKind k : kFlattenAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<FlattenAttr> e = from_base(std::move(eptr))) {
...
std::optional<FlattenAttr> FlattenAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<FlattenAttr> FlattenAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<FlattenAttr> FlattenAttr::from(const VariantEntity &e) {
