...
std::optional<SYCLUniqueStableNameExpr> SYCLUniqueStableNameExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<SYCLUniqueStableNameExpr, ir::hl::Operation>> SYCLUniqueStableNameExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kSYCLUniqueStableNameExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SYCLUniqueStableNameExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
