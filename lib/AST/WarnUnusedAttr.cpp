...
gap::generator<WarnUnusedAttr> WarnUnusedAttr::in(const Index &index) {
  ...
  for (AttrKind k : kWarnUnusedAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<WarnUnusedAttr> e = from_base(std::move(eptr))) {
...
std::optional<WarnUnusedAttr> WarnUnusedAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<WarnUnusedAttr> WarnUnusedAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<WarnUnusedAttr> WarnUnusedAttr::from(const VariantEntity &e) {
