...
std::optional<CharacterLiteral> CharacterLiteral::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CharacterLiteral, ir::hl::Operation>> CharacterLiteral::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCharacterLiteralDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CharacterLiteral, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
