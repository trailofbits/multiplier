...
std::optional<CharacterLiteral> CharacterLiteral::from(const ir::Operation &op) {
...
gap::generator<std::pair<CharacterLiteral, ir::Operation>> CharacterLiteral::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kCharacterLiteralDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CharacterLiteral, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
std::optional<CharacterLiteral> CharacterLiteral::from(const std::optional<Stmt> &parent) {
  if (parent) {
...
std::optional<CharacterLiteral> CharacterLiteral::from_base(const Stmt &parent) {
  switch (parent.kind()) {
...
gap::generator<CharacterLiteral> CharacterLiteral::in(const Index &index) {
  ...
  for (StmtKind k : kCharacterLiteralDerivedKinds) {
    for (StmtImplPtr eptr : ep->StmtsFor(ep, k)) {
      if (std::optional<CharacterLiteral> e = from_base(std::move(eptr))) {
...
std::optional<CharacterLiteral> CharacterLiteral::from(const VariantEntity &e) {
...
CharacterLiteralKind CharacterLiteral::literal_kind(void) const {
  return static_cast<CharacterLiteralKind>(impl->reader.getVal91());
}
