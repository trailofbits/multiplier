...
std::optional<SwiftPrivateAttr> SwiftPrivateAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<SwiftPrivateAttr> SwiftPrivateAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<SwiftPrivateAttr> SwiftPrivateAttr::in(const Index &index) {
  ...
  for (AttrKind k : kSwiftPrivateAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<SwiftPrivateAttr> e = from_base(std::move(eptr))) {
...
std::optional<SwiftPrivateAttr> SwiftPrivateAttr::from(const VariantEntity &e) {
