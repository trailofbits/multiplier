...
gap::generator<ImplicitParamDecl> ImplicitParamDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kImplicitParamDeclDerivedKinds)) {
...
ImplicitParamKind ImplicitParamDecl::parameter_kind(void) const {
  return static_cast<ImplicitParamKind>(impl->reader.getVal119());
}
