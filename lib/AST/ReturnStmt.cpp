...
std::optional<ReturnStmt> ReturnStmt::from(const ir::Operation &op) {
...
gap::generator<std::pair<ReturnStmt, ir::Operation>> ReturnStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kReturnStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ReturnStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
