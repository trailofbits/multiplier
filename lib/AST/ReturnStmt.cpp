...
std::optional<ReturnStmt> ReturnStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ReturnStmt, ir::hl::Operation>> ReturnStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kReturnStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ReturnStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
std::optional<ReturnStmt> ReturnStmt::from(const std::optional<Stmt> &parent) {
  if (parent) {
...
std::optional<ReturnStmt> ReturnStmt::from_base(const Stmt &parent) {
  switch (parent.kind()) {
...
gap::generator<ReturnStmt> ReturnStmt::in(const Index &index) {
  ...
  for (StmtKind k : kReturnStmtDerivedKinds) {
    for (StmtImplPtr eptr : ep->StmtsFor(ep, k)) {
      if (std::optional<ReturnStmt> e = from_base(std::move(eptr))) {
...
std::optional<ReturnStmt> ReturnStmt::from(const VariantEntity &e) {
