...
std::optional<CoroutineBodyStmt> CoroutineBodyStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CoroutineBodyStmt, ir::hl::Operation>> CoroutineBodyStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCoroutineBodyStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CoroutineBodyStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
