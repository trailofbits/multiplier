...
std::optional<OMPTeamsDirective> OMPTeamsDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPTeamsDirective, ir::hl::Operation>> OMPTeamsDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPTeamsDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTeamsDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
std::optional<OMPTeamsDirective> OMPTeamsDirective::from(const std::optional<Stmt> &parent) {
  if (parent) {
...
std::optional<OMPTeamsDirective> OMPTeamsDirective::from_base(const Stmt &parent) {
  switch (parent.kind()) {
...
gap::generator<OMPTeamsDirective> OMPTeamsDirective::in(const Index &index) {
  ...
  for (StmtKind k : kOMPTeamsDirectiveDerivedKinds) {
    for (StmtImplPtr eptr : ep->StmtsFor(ep, k)) {
      if (std::optional<OMPTeamsDirective> e = from_base(std::move(eptr))) {
...
std::optional<OMPTeamsDirective> OMPTeamsDirective::from(const VariantEntity &e) {
