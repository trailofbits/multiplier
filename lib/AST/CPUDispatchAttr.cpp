...
std::optional<CPUDispatchAttr> CPUDispatchAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<CPUDispatchAttr> CPUDispatchAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<CPUDispatchAttr> CPUDispatchAttr::in(const Index &index) {
  ...
  for (AttrKind k : kCPUDispatchAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<CPUDispatchAttr> e = from_base(std::move(eptr))) {
...
std::optional<CPUDispatchAttr> CPUDispatchAttr::from(const VariantEntity &e) {
