...
std::optional<OMPInteropDirective> OMPInteropDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPInteropDirective, ir::hl::Operation>> OMPInteropDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPInteropDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPInteropDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
