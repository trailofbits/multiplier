...
std::optional<OMPTargetDirective> OMPTargetDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPTargetDirective, ir::hl::Operation>> OMPTargetDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPTargetDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTargetDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
std::optional<OMPTargetDirective> OMPTargetDirective::from(const std::optional<Stmt> &parent) {
  if (parent) {
...
std::optional<OMPTargetDirective> OMPTargetDirective::from_base(const Stmt &parent) {
  switch (parent.kind()) {
...
gap::generator<OMPTargetDirective> OMPTargetDirective::in(const Index &index) {
  ...
  for (StmtKind k : kOMPTargetDirectiveDerivedKinds) {
    for (StmtImplPtr eptr : ep->StmtsFor(ep, k)) {
      if (std::optional<OMPTargetDirective> e = from_base(std::move(eptr))) {
...
std::optional<OMPTargetDirective> OMPTargetDirective::from(const VariantEntity &e) {
