...
std::optional<NullStmt> NullStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<NullStmt, ir::hl::Operation>> NullStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kNullStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<NullStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
