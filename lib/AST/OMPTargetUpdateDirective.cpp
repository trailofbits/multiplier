...
std::optional<OMPTargetUpdateDirective> OMPTargetUpdateDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPTargetUpdateDirective, ir::hl::Operation>> OMPTargetUpdateDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPTargetUpdateDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTargetUpdateDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
