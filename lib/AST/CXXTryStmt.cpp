...
std::optional<CXXTryStmt> CXXTryStmt::from(const ir::Operation &op) {
...
gap::generator<std::pair<CXXTryStmt, ir::Operation>> CXXTryStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kCXXTryStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXTryStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
