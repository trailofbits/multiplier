...
gap::generator<CXXTryStmt> CXXTryStmt::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kCXXTryStmtDerivedKinds)) {
...
gap::generator<CXXCatchStmt> CXXTryStmt::handlers(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d16 = ep->StmtFor(ep, v)) {
      if (auto e = CXXCatchStmt::from_base(std::move(d16))) {
        co_yield std::move(*e);
      }
    }
  }
