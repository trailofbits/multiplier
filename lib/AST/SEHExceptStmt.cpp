...
std::optional<SEHExceptStmt> SEHExceptStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<SEHExceptStmt, ir::hl::Operation>> SEHExceptStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kSEHExceptStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SEHExceptStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
