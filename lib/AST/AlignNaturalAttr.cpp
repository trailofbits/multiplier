...
gap::generator<AlignNaturalAttr> AlignNaturalAttr::in(const Index &index) {
  ...
  for (AttrKind k : kAlignNaturalAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<AlignNaturalAttr> e = from_base(std::move(eptr))) {
...
std::optional<AlignNaturalAttr> AlignNaturalAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<AlignNaturalAttr> AlignNaturalAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<AlignNaturalAttr> AlignNaturalAttr::from(const VariantEntity &e) {
