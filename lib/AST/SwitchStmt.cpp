...
std::optional<SwitchStmt> SwitchStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<SwitchStmt, ir::hl::Operation>> SwitchStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kSwitchStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SwitchStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
