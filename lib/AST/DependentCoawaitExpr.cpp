...
std::optional<DependentCoawaitExpr> DependentCoawaitExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<DependentCoawaitExpr, ir::hl::Operation>> DependentCoawaitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kDependentCoawaitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DependentCoawaitExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
