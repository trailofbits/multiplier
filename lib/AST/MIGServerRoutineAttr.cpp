...
gap::generator<MIGServerRoutineAttr> MIGServerRoutineAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMIGServerRoutineAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MIGServerRoutineAttr> e = from_base(std::move(eptr))) {
...
std::optional<MIGServerRoutineAttr> MIGServerRoutineAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MIGServerRoutineAttr> MIGServerRoutineAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<MIGServerRoutineAttr> MIGServerRoutineAttr::from(const VariantEntity &e) {
