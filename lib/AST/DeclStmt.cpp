...
std::optional<DeclStmt> DeclStmt::from(const ir::Operation &op) {
...
gap::generator<std::pair<DeclStmt, ir::Operation>> DeclStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kDeclStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DeclStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
