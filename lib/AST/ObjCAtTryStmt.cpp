...
gap::generator<ObjCAtTryStmt> ObjCAtTryStmt::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kObjCAtTryStmtDerivedKinds)) {
...
gap::generator<ObjCAtCatchStmt> ObjCAtTryStmt::catch_statements(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d16 = ep->StmtFor(ep, v)) {
      if (auto e = ObjCAtCatchStmt::from_base(std::move(d16))) {
        co_yield std::move(*e);
      }
    }
  }
