...
std::optional<ObjCIndirectCopyRestoreExpr> ObjCIndirectCopyRestoreExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ObjCIndirectCopyRestoreExpr, ir::hl::Operation>> ObjCIndirectCopyRestoreExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kObjCIndirectCopyRestoreExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCIndirectCopyRestoreExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
