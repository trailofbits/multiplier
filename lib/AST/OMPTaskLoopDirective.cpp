...
std::optional<OMPTaskLoopDirective> OMPTaskLoopDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPTaskLoopDirective, ir::hl::Operation>> OMPTaskLoopDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPTaskLoopDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTaskLoopDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
