...
std::optional<ConvertVectorExpr> ConvertVectorExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<ConvertVectorExpr, ir::Operation>> ConvertVectorExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kConvertVectorExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ConvertVectorExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
