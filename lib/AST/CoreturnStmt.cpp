...
std::optional<CoreturnStmt> CoreturnStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CoreturnStmt, ir::hl::Operation>> CoreturnStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCoreturnStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CoreturnStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
std::optional<CoreturnStmt> CoreturnStmt::from(const std::optional<Stmt> &parent) {
  if (parent) {
...
std::optional<CoreturnStmt> CoreturnStmt::from_base(const Stmt &parent) {
  switch (parent.kind()) {
...
gap::generator<CoreturnStmt> CoreturnStmt::in(const Index &index) {
  ...
  for (StmtKind k : kCoreturnStmtDerivedKinds) {
    for (StmtImplPtr eptr : ep->StmtsFor(ep, k)) {
      if (std::optional<CoreturnStmt> e = from_base(std::move(eptr))) {
...
std::optional<CoreturnStmt> CoreturnStmt::from(const VariantEntity &e) {
