...
std::optional<TypeAliasTemplateDecl> TypeAliasTemplateDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<TypeAliasTemplateDecl, ir::hl::Operation>> TypeAliasTemplateDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kTypeAliasTemplateDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<TypeAliasTemplateDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
