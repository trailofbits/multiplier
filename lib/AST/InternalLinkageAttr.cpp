...
std::optional<InternalLinkageAttr> InternalLinkageAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<InternalLinkageAttr> InternalLinkageAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<InternalLinkageAttr> InternalLinkageAttr::in(const Index &index) {
  ...
  for (AttrKind k : kInternalLinkageAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<InternalLinkageAttr> e = from_base(std::move(eptr))) {
...
std::optional<InternalLinkageAttr> InternalLinkageAttr::from(const VariantEntity &e) {
