...
std::optional<InitListExpr> InitListExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<InitListExpr, ir::hl::Operation>> InitListExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kInitListExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<InitListExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
