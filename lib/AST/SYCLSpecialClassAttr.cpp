...
std::optional<SYCLSpecialClassAttr> SYCLSpecialClassAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<SYCLSpecialClassAttr> SYCLSpecialClassAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<SYCLSpecialClassAttr> SYCLSpecialClassAttr::in(const Index &index) {
  ...
  for (AttrKind k : kSYCLSpecialClassAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<SYCLSpecialClassAttr> e = from_base(std::move(eptr))) {
...
std::optional<SYCLSpecialClassAttr> SYCLSpecialClassAttr::from(const VariantEntity &e) {
