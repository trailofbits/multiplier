...
std::optional<CompoundStmt> CompoundStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CompoundStmt, ir::hl::Operation>> CompoundStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCompoundStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CompoundStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
