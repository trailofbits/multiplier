...
std::optional<NoInitExpr> NoInitExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<NoInitExpr, ir::hl::Operation>> NoInitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kNoInitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<NoInitExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
