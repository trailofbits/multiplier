...
std::optional<NoInitExpr> NoInitExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<NoInitExpr, ir::Operation>> NoInitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kNoInitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<NoInitExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
