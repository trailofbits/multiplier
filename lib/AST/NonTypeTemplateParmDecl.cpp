...
gap::generator<NonTypeTemplateParmDecl> NonTypeTemplateParmDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kNonTypeTemplateParmDeclDerivedKinds)) {
...
gap::generator<Type> NonTypeTemplateParmDecl::expansion_types(void) const & {
  ...
  for (auto v : list) {
    EntityId id(v);
    if (auto d44 = ep->TypeFor(ep, v)) {
      co_yield Type(std::move(d44));
    }
  }
  co_return;
}
