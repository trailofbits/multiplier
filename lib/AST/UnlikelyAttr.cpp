...
gap::generator<UnlikelyAttr> UnlikelyAttr::in(const Index &index) {
  ...
  for (AttrKind k : kUnlikelyAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<UnlikelyAttr> e = from_base(std::move(eptr))) {
...
gap::generator<UnlikelyAttr> UnlikelyAttr::in(const File &file) {
...
std::optional<UnlikelyAttr> UnlikelyAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<UnlikelyAttr> UnlikelyAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<UnlikelyAttr> UnlikelyAttr::from(const VariantEntity &e) {
