...
std::optional<VAArgExpr> VAArgExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<VAArgExpr, ir::hl::Operation>> VAArgExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kVAArgExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<VAArgExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
