...
std::optional<DoStmt> DoStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<DoStmt, ir::hl::Operation>> DoStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kDoStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DoStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
