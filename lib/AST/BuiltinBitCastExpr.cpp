...
std::optional<BuiltinBitCastExpr> BuiltinBitCastExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<BuiltinBitCastExpr, ir::Operation>> BuiltinBitCastExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kBuiltinBitCastExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<BuiltinBitCastExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
