...
gap::generator<OMPThreadPrivateDecl> OMPThreadPrivateDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kOMPThreadPrivateDeclDerivedKinds)) {
...
gap::generator<Expr> OMPThreadPrivateDecl::varlists(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d44 = ep->StmtFor(ep, v)) {
      if (auto e = Expr::from_base(std::move(d44))) {
        co_yield std::move(*e);
      }
    }
  }
