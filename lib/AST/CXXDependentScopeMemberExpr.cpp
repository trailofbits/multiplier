...
std::optional<CXXDependentScopeMemberExpr> CXXDependentScopeMemberExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXDependentScopeMemberExpr, ir::hl::Operation>> CXXDependentScopeMemberExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXDependentScopeMemberExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXDependentScopeMemberExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
