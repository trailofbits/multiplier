...
std::optional<OMPTeamsDistributeSimdDirective> OMPTeamsDistributeSimdDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPTeamsDistributeSimdDirective, ir::hl::Operation>> OMPTeamsDistributeSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPTeamsDistributeSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTeamsDistributeSimdDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
