...
std::optional<HLSLNumThreadsAttr> HLSLNumThreadsAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
gap::generator<HLSLNumThreadsAttr> HLSLNumThreadsAttr::in(const Index &index) {
  ...
  for (AttrKind k : kHLSLNumThreadsAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<HLSLNumThreadsAttr> e = from_base(std::move(eptr))) {
...
gap::generator<HLSLNumThreadsAttr> HLSLNumThreadsAttr::in(const File &file) {
...
std::optional<HLSLNumThreadsAttr> HLSLNumThreadsAttr::from(const VariantEntity &e) {
