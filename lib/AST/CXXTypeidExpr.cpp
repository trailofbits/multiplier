...
std::optional<CXXTypeidExpr> CXXTypeidExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXTypeidExpr, ir::hl::Operation>> CXXTypeidExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXTypeidExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXTypeidExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
