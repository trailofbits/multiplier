...
std::optional<CXXStaticCastExpr> CXXStaticCastExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<CXXStaticCastExpr, ir::Operation>> CXXStaticCastExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kCXXStaticCastExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXStaticCastExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
