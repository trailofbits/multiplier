...
std::optional<ObjCPropertyImplDecl> ObjCPropertyImplDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ObjCPropertyImplDecl, ir::hl::Operation>> ObjCPropertyImplDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kObjCPropertyImplDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCPropertyImplDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
