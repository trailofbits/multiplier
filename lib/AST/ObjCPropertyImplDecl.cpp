...
std::optional<ObjCPropertyImplDecl> ObjCPropertyImplDecl::from(const ir::Operation &op) {
...
gap::generator<std::pair<ObjCPropertyImplDecl, ir::Operation>> ObjCPropertyImplDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kObjCPropertyImplDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCPropertyImplDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
ObjCPropertyImplDeclKind ObjCPropertyImplDecl::property_implementation(void) const {
  return static_cast<ObjCPropertyImplDeclKind>(impl->reader.getVal54());
}
