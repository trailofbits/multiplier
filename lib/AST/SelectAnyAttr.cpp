...
std::optional<SelectAnyAttr> SelectAnyAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<SelectAnyAttr> SelectAnyAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<SelectAnyAttr> SelectAnyAttr::in(const Index &index) {
  ...
  for (AttrKind k : kSelectAnyAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<SelectAnyAttr> e = from_base(std::move(eptr))) {
...
std::optional<SelectAnyAttr> SelectAnyAttr::from(const VariantEntity &e) {
