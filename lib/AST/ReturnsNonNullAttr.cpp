...
gap::generator<ReturnsNonNullAttr> ReturnsNonNullAttr::in(const Index &index) {
  ...
  for (AttrKind k : kReturnsNonNullAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ReturnsNonNullAttr> e = from_base(std::move(eptr))) {
...
std::optional<ReturnsNonNullAttr> ReturnsNonNullAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ReturnsNonNullAttr> ReturnsNonNullAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<ReturnsNonNullAttr> ReturnsNonNullAttr::from(const VariantEntity &e) {
