...
gap::generator<ObjCRequiresSuperAttr> ObjCRequiresSuperAttr::in(const Index &index) {
  ...
  for (AttrKind k : kObjCRequiresSuperAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ObjCRequiresSuperAttr> e = from_base(std::move(eptr))) {
...
std::optional<ObjCRequiresSuperAttr> ObjCRequiresSuperAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ObjCRequiresSuperAttr> ObjCRequiresSuperAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<ObjCRequiresSuperAttr> ObjCRequiresSuperAttr::from(const VariantEntity &e) {
