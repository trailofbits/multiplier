...
gap::generator<CXXOperatorCallExpr> CXXOperatorCallExpr::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kCXXOperatorCallExprDerivedKinds)) {
...
OverloadedOperatorKind CXXOperatorCallExpr::operator_(void) const {
  return static_cast<OverloadedOperatorKind>(impl->reader.getVal91());
}
