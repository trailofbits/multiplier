...
std::optional<MSStructAttr> MSStructAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MSStructAttr> MSStructAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<MSStructAttr> MSStructAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMSStructAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MSStructAttr> e = from_base(std::move(eptr))) {
...
std::optional<MSStructAttr> MSStructAttr::from(const VariantEntity &e) {
