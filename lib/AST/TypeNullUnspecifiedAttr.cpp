...
gap::generator<TypeNullUnspecifiedAttr> TypeNullUnspecifiedAttr::in(const Index &index) {
  ...
  for (AttrKind k : kTypeNullUnspecifiedAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<TypeNullUnspecifiedAttr> e = from_base(std::move(eptr))) {
...
gap::generator<TypeNullUnspecifiedAttr> TypeNullUnspecifiedAttr::in(const File &file) {
...
std::optional<TypeNullUnspecifiedAttr> TypeNullUnspecifiedAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<TypeNullUnspecifiedAttr> TypeNullUnspecifiedAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<TypeNullUnspecifiedAttr> TypeNullUnspecifiedAttr::from(const VariantEntity &e) {
