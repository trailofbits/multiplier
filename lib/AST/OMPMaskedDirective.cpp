...
std::optional<OMPMaskedDirective> OMPMaskedDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPMaskedDirective, ir::hl::Operation>> OMPMaskedDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPMaskedDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPMaskedDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
