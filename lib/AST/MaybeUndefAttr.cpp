...
std::optional<MaybeUndefAttr> MaybeUndefAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MaybeUndefAttr> MaybeUndefAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<MaybeUndefAttr> MaybeUndefAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMaybeUndefAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MaybeUndefAttr> e = from_base(std::move(eptr))) {
...
std::optional<MaybeUndefAttr> MaybeUndefAttr::from(const VariantEntity &e) {
