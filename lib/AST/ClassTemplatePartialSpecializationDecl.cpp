...
std::optional<ClassTemplatePartialSpecializationDecl> ClassTemplatePartialSpecializationDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ClassTemplatePartialSpecializationDecl, ir::hl::Operation>> ClassTemplatePartialSpecializationDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kClassTemplatePartialSpecializationDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ClassTemplatePartialSpecializationDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
