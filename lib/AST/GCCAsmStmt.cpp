...
gap::generator<GCCAsmStmt> GCCAsmStmt::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kGCCAsmStmtDerivedKinds)) {
...
gap::generator<AddrLabelExpr> GCCAsmStmt::labels(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d31 = ep->StmtFor(ep, v)) {
      if (auto e = AddrLabelExpr::from_base(std::move(d31))) {
        co_yield std::move(*e);
      }
    }
  }
...
gap::generator<StringLiteral> GCCAsmStmt::input_constraint_literals(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d55 = ep->StmtFor(ep, v)) {
      if (auto e = StringLiteral::from_base(std::move(d55))) {
        co_yield std::move(*e);
      }
    }
  }
...
gap::generator<StringLiteral> GCCAsmStmt::clobber_string_literals(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d56 = ep->StmtFor(ep, v)) {
      if (auto e = StringLiteral::from_base(std::move(d56))) {
        co_yield std::move(*e);
      }
    }
  }
...
unsigned GCCAsmStmt::num_label_expressions(void) const {
  return impl->reader.getVal69().size();
}
...
gap::generator<AddrLabelExpr> GCCAsmStmt::label_expressions(void) const & {
  ...
  for (auto v : list) {
    EntityId id(v);
    if (auto d69 = ep->StmtFor(ep, v)) {
      if (auto e = AddrLabelExpr::from_base(std::move(d69))) {
        co_yield std::move(*e);
      }
    }
  }
