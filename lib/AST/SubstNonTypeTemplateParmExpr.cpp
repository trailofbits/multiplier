...
std::optional<SubstNonTypeTemplateParmExpr> SubstNonTypeTemplateParmExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<SubstNonTypeTemplateParmExpr, ir::hl::Operation>> SubstNonTypeTemplateParmExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kSubstNonTypeTemplateParmExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SubstNonTypeTemplateParmExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
