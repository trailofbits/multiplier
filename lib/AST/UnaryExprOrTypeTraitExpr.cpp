...
std::optional<UnaryExprOrTypeTraitExpr> UnaryExprOrTypeTraitExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<UnaryExprOrTypeTraitExpr, ir::Operation>> UnaryExprOrTypeTraitExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kUnaryExprOrTypeTraitExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<UnaryExprOrTypeTraitExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
UnaryExprOrTypeTrait UnaryExprOrTypeTraitExpr::keyword_kind(void) const {
  return static_cast<UnaryExprOrTypeTrait>(impl->reader.getVal89());
}
