...
std::optional<MSABIAttr> MSABIAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<MSABIAttr> MSABIAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<MSABIAttr> MSABIAttr::in(const Index &index) {
  ...
  for (AttrKind k : kMSABIAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<MSABIAttr> e = from_base(std::move(eptr))) {
...
std::optional<MSABIAttr> MSABIAttr::from(const VariantEntity &e) {
