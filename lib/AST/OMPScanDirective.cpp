...
std::optional<OMPScanDirective> OMPScanDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPScanDirective, ir::hl::Operation>> OMPScanDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPScanDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPScanDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
