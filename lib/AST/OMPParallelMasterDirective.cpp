...
std::optional<OMPParallelMasterDirective> OMPParallelMasterDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPParallelMasterDirective, ir::Operation>> OMPParallelMasterDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPParallelMasterDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPParallelMasterDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
