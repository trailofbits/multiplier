...
std::optional<ConditionalOperator> ConditionalOperator::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ConditionalOperator, ir::hl::Operation>> ConditionalOperator::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kConditionalOperatorDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ConditionalOperator, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
