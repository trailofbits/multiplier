...
gap::generator<ObjCProtocolDecl> ObjCProtocolDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kObjCProtocolDeclDerivedKinds)) {
...
std::string_view ObjCProtocolDecl::obj_c_runtime_name_as_string(void) const {
  capnp::Text::Reader data = impl->reader.getVal57();
...
unsigned ObjCProtocolDecl::num_protocol_tokens(void) const {
  return impl->reader.getVal330().size();
}
...
gap::generator<Token> ObjCProtocolDecl::protocol_tokens(void) const & {
  ...
  for (auto v : list) {
    EntityId id(v);
    if (auto t330 = ep->TokenFor(ep, tok_reader, v)) {
      co_yield t330;
    }
  }
  co_return;
