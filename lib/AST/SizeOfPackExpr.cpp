...
gap::generator<SizeOfPackExpr> SizeOfPackExpr::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kSizeOfPackExprDerivedKinds)) {
...
std::optional<uint32_t> SizeOfPackExpr::pack_length(void) const {
  if (!impl->reader.getVal85()) {
    ...
    return static_cast<uint32_t>(impl->reader.getVal27());
  }
  return std::nullopt;
}
