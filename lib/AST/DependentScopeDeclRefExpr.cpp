...
std::optional<DependentScopeDeclRefExpr> DependentScopeDeclRefExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<DependentScopeDeclRefExpr, ir::Operation>> DependentScopeDeclRefExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kDependentScopeDeclRefExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<DependentScopeDeclRefExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
