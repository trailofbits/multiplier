...
std::optional<ConstAttr> ConstAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ConstAttr> ConstAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<ConstAttr> ConstAttr::in(const Index &index) {
  ...
  for (AttrKind k : kConstAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ConstAttr> e = from_base(std::move(eptr))) {
...
gap::generator<ConstAttr> ConstAttr::in(const File &file) {
...
std::optional<ConstAttr> ConstAttr::from(const Reference &r) {
  return ConstAttr::from(r.as_attribute());
}

std::optional<ConstAttr> ConstAttr::from(const VariantEntity &e) {
...
std::optional<ConstAttr> ConstAttr::from(const TokenContext &t) {
