...
std::optional<LambdaExpr> LambdaExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<LambdaExpr, ir::hl::Operation>> LambdaExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kLambdaExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<LambdaExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
