...
std::optional<ChooseExpr> ChooseExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ChooseExpr, ir::hl::Operation>> ChooseExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kChooseExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ChooseExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
