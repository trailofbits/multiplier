...
std::optional<ChooseExpr> ChooseExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<ChooseExpr, ir::Operation>> ChooseExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kChooseExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ChooseExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
