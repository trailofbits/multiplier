...
gap::generator<std::pair<ObjCAtCatchStmt, ir::Operation>> ObjCAtCatchStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kObjCAtCatchStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCAtCatchStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
