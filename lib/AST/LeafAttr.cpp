...
gap::generator<LeafAttr> LeafAttr::in(const Index &index) {
  ...
  for (AttrKind k : kLeafAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<LeafAttr> e = from_base(std::move(eptr))) {
...
gap::generator<LeafAttr> LeafAttr::in(const File &file) {
...
std::optional<LeafAttr> LeafAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<LeafAttr> LeafAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<LeafAttr> LeafAttr::from(const VariantEntity &e) {
