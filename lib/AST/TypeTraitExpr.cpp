...
gap::generator<TypeTraitExpr> TypeTraitExpr::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kTypeTraitExprDerivedKinds)) {
...
TypeTrait TypeTraitExpr::trait(void) const {
  return static_cast<TypeTrait>(impl->reader.getVal89());
}
