...
std::optional<CXXMemberCallExpr> CXXMemberCallExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXMemberCallExpr, ir::hl::Operation>> CXXMemberCallExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXMemberCallExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXMemberCallExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
