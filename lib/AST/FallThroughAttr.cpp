...
std::optional<FallThroughAttr> FallThroughAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<FallThroughAttr> FallThroughAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<FallThroughAttr> FallThroughAttr::in(const Index &index) {
  ...
  for (AttrKind k : kFallThroughAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<FallThroughAttr> e = from_base(std::move(eptr))) {
...
std::optional<FallThroughAttr> FallThroughAttr::from(const VariantEntity &e) {
