...
std::optional<DeclOrStmtAttr> DeclOrStmtAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
gap::generator<DeclOrStmtAttr> DeclOrStmtAttr::in(const Index &index) {
  ...
  for (AttrKind k : kDeclOrStmtAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<DeclOrStmtAttr> e = from_base(std::move(eptr))) {
...
std::optional<DeclOrStmtAttr> DeclOrStmtAttr::from(const VariantEntity &e) {
