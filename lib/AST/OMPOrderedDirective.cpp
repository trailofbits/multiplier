...
gap::generator<std::pair<OMPOrderedDirective, ir::Operation>> OMPOrderedDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPOrderedDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPOrderedDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
