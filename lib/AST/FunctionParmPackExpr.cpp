...
gap::generator<FunctionParmPackExpr> FunctionParmPackExpr::in(const Compilation &tu) {
  for (Stmt res : Stmt::in(tu, kFunctionParmPackExprDerivedKinds)) {
...
gap::generator<VarDecl> FunctionParmPackExpr::expansions(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d16 = ep->DeclFor(ep, v)) {
      if (auto e = VarDecl::from_base(std::move(d16))) {
        co_yield std::move(*e);
      }
    }
  }
