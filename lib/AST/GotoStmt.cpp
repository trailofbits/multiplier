...
std::optional<GotoStmt> GotoStmt::from(const ir::Operation &op) {
...
gap::generator<std::pair<GotoStmt, ir::Operation>> GotoStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kGotoStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<GotoStmt, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
