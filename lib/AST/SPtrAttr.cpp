...
std::optional<SPtrAttr> SPtrAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<SPtrAttr> SPtrAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<SPtrAttr> SPtrAttr::in(const Index &index) {
  ...
  for (AttrKind k : kSPtrAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<SPtrAttr> e = from_base(std::move(eptr))) {
...
gap::generator<SPtrAttr> SPtrAttr::in(const File &file) {
...
std::optional<SPtrAttr> SPtrAttr::from(const VariantEntity &e) {
