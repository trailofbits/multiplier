...
std::optional<OMPAtomicDirective> OMPAtomicDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPAtomicDirective, ir::hl::Operation>> OMPAtomicDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPAtomicDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPAtomicDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
