...
std::optional<ObjCPropertyDecl> ObjCPropertyDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ObjCPropertyDecl, ir::hl::Operation>> ObjCPropertyDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kObjCPropertyDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCPropertyDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
