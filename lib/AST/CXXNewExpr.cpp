...
std::optional<CXXNewExpr> CXXNewExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXNewExpr, ir::hl::Operation>> CXXNewExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXNewExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXNewExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
