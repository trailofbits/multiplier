...
gap::generator<SentinelAttr> SentinelAttr::in(const Index &index) {
  ...
  for (AttrKind k : kSentinelAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<SentinelAttr> e = from_base(std::move(eptr))) {
...
gap::generator<SentinelAttr> SentinelAttr::in(const File &file) {
...
std::optional<SentinelAttr> SentinelAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<SentinelAttr> SentinelAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<SentinelAttr> SentinelAttr::from(const VariantEntity &e) {
