...
std::optional<ExprWithCleanups> ExprWithCleanups::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ExprWithCleanups, ir::hl::Operation>> ExprWithCleanups::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kExprWithCleanupsDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ExprWithCleanups, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
