...
std::optional<OMPTargetTeamsDistributeSimdDirective> OMPTargetTeamsDistributeSimdDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPTargetTeamsDistributeSimdDirective, ir::Operation>> OMPTargetTeamsDistributeSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPTargetTeamsDistributeSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTargetTeamsDistributeSimdDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
