...
std::optional<StringLiteral> StringLiteral::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<StringLiteral, ir::hl::Operation>> StringLiteral::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kStringLiteralDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<StringLiteral, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
