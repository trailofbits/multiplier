...
std::optional<UnsafeBufferUsageAttr> UnsafeBufferUsageAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<UnsafeBufferUsageAttr> UnsafeBufferUsageAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<UnsafeBufferUsageAttr> UnsafeBufferUsageAttr::in(const Index &index) {
  ...
  for (AttrKind k : kUnsafeBufferUsageAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<UnsafeBufferUsageAttr> e = from_base(std::move(eptr))) {
...
std::optional<UnsafeBufferUsageAttr> UnsafeBufferUsageAttr::from(const VariantEntity &e) {
