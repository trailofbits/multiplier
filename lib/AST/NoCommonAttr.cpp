...
gap::generator<NoCommonAttr> NoCommonAttr::in(const Index &index) {
  ...
  for (AttrKind k : kNoCommonAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<NoCommonAttr> e = from_base(std::move(eptr))) {
...
gap::generator<NoCommonAttr> NoCommonAttr::in(const File &file) {
...
std::optional<NoCommonAttr> NoCommonAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<NoCommonAttr> NoCommonAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<NoCommonAttr> NoCommonAttr::from(const VariantEntity &e) {
