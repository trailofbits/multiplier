...
std::optional<CXXThrowExpr> CXXThrowExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CXXThrowExpr, ir::hl::Operation>> CXXThrowExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kCXXThrowExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXThrowExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
