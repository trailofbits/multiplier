...
std::optional<OMPIteratorExpr> OMPIteratorExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPIteratorExpr, ir::hl::Operation>> OMPIteratorExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPIteratorExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPIteratorExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
