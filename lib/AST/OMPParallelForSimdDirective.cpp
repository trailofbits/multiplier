...
std::optional<OMPParallelForSimdDirective> OMPParallelForSimdDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPParallelForSimdDirective, ir::hl::Operation>> OMPParallelForSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPParallelForSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPParallelForSimdDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
