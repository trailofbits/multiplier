...
gap::generator<CapturedDecl> CapturedDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kCapturedDeclDerivedKinds)) {
...
gap::generator<ImplicitParamDecl> CapturedDecl::parameters(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d44 = ep->DeclFor(ep, v)) {
      if (auto e = ImplicitParamDecl::from_base(std::move(d44))) {
        co_yield std::move(*e);
      }
    }
  }
