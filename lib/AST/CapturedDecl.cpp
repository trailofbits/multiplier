...
std::optional<CapturedDecl> CapturedDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<CapturedDecl, ir::hl::Operation>> CapturedDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kCapturedDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CapturedDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
