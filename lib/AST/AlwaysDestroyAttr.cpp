...
gap::generator<AlwaysDestroyAttr> AlwaysDestroyAttr::in(const Index &index) {
  ...
  for (AttrKind k : kAlwaysDestroyAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<AlwaysDestroyAttr> e = from_base(std::move(eptr))) {
...
gap::generator<AlwaysDestroyAttr> AlwaysDestroyAttr::in(const File &file) {
...
std::optional<AlwaysDestroyAttr> AlwaysDestroyAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<AlwaysDestroyAttr> AlwaysDestroyAttr::from(const VariantEntity &e) {
