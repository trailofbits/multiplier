...
std::optional<CXXPseudoDestructorExpr> CXXPseudoDestructorExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<CXXPseudoDestructorExpr, ir::Operation>> CXXPseudoDestructorExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kCXXPseudoDestructorExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<CXXPseudoDestructorExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
