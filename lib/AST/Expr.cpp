...
std::optional<Expr> Expr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<Expr, ir::hl::Operation>> Expr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<Expr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
ExprObjectKind Expr::object_kind(void) const {
  return static_cast<ExprObjectKind>(impl->reader.getVal69());
}
...
std::optional<bool> Expr::is_integer_constant_expression(void) const {
  if (!impl->reader.getVal72()) {
    ...
    return static_cast<bool>(impl->reader.getVal71());
  }
  return std::nullopt;
}
