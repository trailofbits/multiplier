...
gap::generator<CommonAttr> CommonAttr::in(const Index &index) {
  ...
  for (AttrKind k : kCommonAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<CommonAttr> e = from_base(std::move(eptr))) {
...
gap::generator<CommonAttr> CommonAttr::in(const File &file) {
...
std::optional<CommonAttr> CommonAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<CommonAttr> CommonAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<CommonAttr> CommonAttr::from(const VariantEntity &e) {
