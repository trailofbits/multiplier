...
std::optional<InitPriorityAttr> InitPriorityAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<InitPriorityAttr> InitPriorityAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<InitPriorityAttr> InitPriorityAttr::in(const Index &index) {
  ...
  for (AttrKind k : kInitPriorityAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<InitPriorityAttr> e = from_base(std::move(eptr))) {
...
gap::generator<InitPriorityAttr> InitPriorityAttr::in(const File &file) {
...
std::optional<InitPriorityAttr> InitPriorityAttr::from(const VariantEntity &e) {
