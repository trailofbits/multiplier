...
std::optional<OMPMaskedTaskLoopSimdDirective> OMPMaskedTaskLoopSimdDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPMaskedTaskLoopSimdDirective, ir::hl::Operation>> OMPMaskedTaskLoopSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPMaskedTaskLoopSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPMaskedTaskLoopSimdDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
