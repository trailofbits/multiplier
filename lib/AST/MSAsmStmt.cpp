...
std::optional<MSAsmStmt> MSAsmStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<MSAsmStmt, ir::hl::Operation>> MSAsmStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kMSAsmStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<MSAsmStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
