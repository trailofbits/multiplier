...
gap::generator<CXXRecordDecl> CXXRecordDecl::in(const Compilation &tu) {
  for (Decl res : Decl::in(tu, kCXXRecordDeclDerivedKinds)) {
...
std::optional<bool> CXXRecordDecl::allow_const_default_initializer(void) const {
  if (!impl->reader.getVal124()) {
    ...
    return static_cast<bool>(impl->reader.getVal123());
  }
  return std::nullopt;
}
...
std::optional<std::vector<CXXBaseSpecifier>> CXXRecordDecl::bases(void) const {
  ...
  auto list = impl->reader.getVal154();
  std::vector<CXXBaseSpecifier> vec;
  vec.reserve(list.size());
  EntityProviderPtr ep = impl->ep;
  for (auto v : list) {
    EntityId id(v);
    if (auto d154 = ep->CXXBaseSpecifierFor(ep, v)) {
      vec.emplace_back(std::move(d154));
    }
  }
  return vec;
}
...
std::optional<MSInheritanceModel> CXXRecordDecl::inheritance_model(void) const {
  if (!impl->reader.getVal126()) {
    ...
    return static_cast<MSInheritanceModel>(impl->reader.getVal78());
  }
  return std::nullopt;
}
...
gap::generator<CXXConstructorDecl> CXXRecordDecl::constructors(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d169 = ep->DeclFor(ep, v)) {
      if (auto e = CXXConstructorDecl::from_base(std::move(d169))) {
        co_yield std::move(*e);
      }
    }
  }
...
std::optional<LambdaCaptureDefault> CXXRecordDecl::lambda_capture_default(void) const {
  if (!impl->reader.getVal128()) {
    ...
    return static_cast<LambdaCaptureDefault>(impl->reader.getVal79());
  }
  return std::nullopt;
}
...
std::optional<std::vector<NamedDecl>> CXXRecordDecl::lambda_explicit_template_parameters(void) const {
  ...
  for (auto v : list) {
    ...
    if (auto d179 = ep->DeclFor(ep, v)) {
      if (auto e = NamedDecl::from_base(std::move(d179))) {
        vec.emplace_back(std::move(*e));
      }
    }
  }
...
std::optional<uint32_t> CXXRecordDecl::lambda_mangling_number(void) const {
  if (!impl->reader.getVal132()) {
    ...
    return static_cast<uint32_t>(impl->reader.getVal118());
  }
  return std::nullopt;
}
...
std::optional<MSInheritanceModel> CXXRecordDecl::ms_inheritance_model(void) const {
  if (!impl->reader.getVal133()) {
    ...
    return static_cast<MSInheritanceModel>(impl->reader.getVal80());
  }
  return std::nullopt;
}

MSVtorDispMode CXXRecordDecl::ms_vtor_disp_mode(void) const {
  return static_cast<MSVtorDispMode>(impl->reader.getVal81());
}
...
std::optional<std::vector<CXXBaseSpecifier>> CXXRecordDecl::virtual_bases(void) const {
  ...
  auto list = impl->reader.getVal330();
  std::vector<CXXBaseSpecifier> vec;
  vec.reserve(list.size());
  EntityProviderPtr ep = impl->ep;
  for (auto v : list) {
    EntityId id(v);
    if (auto d330 = ep->CXXBaseSpecifierFor(ep, v)) {
      vec.emplace_back(std::move(d330));
    }
  }
  return vec;
}
...
std::optional<uint64_t> CXXRecordDecl::size_without_virtual_bases(void) const {
  if (!impl->reader.getVal332()) {
    ...
    return static_cast<uint64_t>(impl->reader.getVal155());
  }
  return std::nullopt;
}
...
std::optional<CXXRecordDecl> CXXRecordDecl::primary_base(void) const {
  if (true) {
    RawEntityId eid = impl->reader.getVal156();
    if (eid == kInvalidEntityId) {
      return std::nullopt;
    }
    if (auto eptr = impl->ep->DeclFor(impl->ep, eid)) {
