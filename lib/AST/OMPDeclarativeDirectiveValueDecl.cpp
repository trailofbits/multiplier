...
gap::generator<std::pair<OMPDeclarativeDirectiveValueDecl, ir::Operation>> OMPDeclarativeDirectiveValueDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kOMPDeclarativeDirectiveValueDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPDeclarativeDirectiveValueDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
