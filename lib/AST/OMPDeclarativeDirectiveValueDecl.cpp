...
std::optional<OMPDeclarativeDirectiveValueDecl> OMPDeclarativeDirectiveValueDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPDeclarativeDirectiveValueDecl, ir::hl::Operation>> OMPDeclarativeDirectiveValueDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kOMPDeclarativeDirectiveValueDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPDeclarativeDirectiveValueDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
