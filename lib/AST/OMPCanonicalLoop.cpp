...
std::optional<OMPCanonicalLoop> OMPCanonicalLoop::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPCanonicalLoop, ir::hl::Operation>> OMPCanonicalLoop::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPCanonicalLoopDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPCanonicalLoop, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
