...
std::optional<SourceLocExpr> SourceLocExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<SourceLocExpr, ir::hl::Operation>> SourceLocExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kSourceLocExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<SourceLocExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
