...
std::optional<ObjCBridgedCastExpr> ObjCBridgedCastExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<ObjCBridgedCastExpr, ir::Operation>> ObjCBridgedCastExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kObjCBridgedCastExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCBridgedCastExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
ObjCBridgeCastKind ObjCBridgedCastExpr::bridge_kind(void) const {
  return static_cast<ObjCBridgeCastKind>(impl->reader.getVal90());
}
