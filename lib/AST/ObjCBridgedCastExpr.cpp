...
std::optional<ObjCBridgedCastExpr> ObjCBridgedCastExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ObjCBridgedCastExpr, ir::hl::Operation>> ObjCBridgedCastExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kObjCBridgedCastExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ObjCBridgedCastExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
