...
gap::generator<std::pair<ParmVarDecl, ir::Operation>> ParmVarDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kParmVarDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ParmVarDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
DeclObjCDeclQualifier ParmVarDecl::obj_c_decl_qualifier(void) const {
  return static_cast<DeclObjCDeclQualifier>(impl->reader.getVal114());
}
