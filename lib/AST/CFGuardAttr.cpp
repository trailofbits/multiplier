...
std::optional<CFGuardAttr> CFGuardAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<CFGuardAttr> CFGuardAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<CFGuardAttr> CFGuardAttr::in(const Index &index) {
  ...
  for (AttrKind k : kCFGuardAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<CFGuardAttr> e = from_base(std::move(eptr))) {
...
std::optional<CFGuardAttr> CFGuardAttr::from(const VariantEntity &e) {
