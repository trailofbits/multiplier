...
std::optional<IndirectFieldDecl> IndirectFieldDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<IndirectFieldDecl, ir::hl::Operation>> IndirectFieldDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kIndirectFieldDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<IndirectFieldDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
