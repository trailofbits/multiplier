...
gap::generator<NoMergeAttr> NoMergeAttr::in(const Index &index) {
  ...
  for (AttrKind k : kNoMergeAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<NoMergeAttr> e = from_base(std::move(eptr))) {
...
gap::generator<NoMergeAttr> NoMergeAttr::in(const File &file) {
...
std::optional<NoMergeAttr> NoMergeAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<NoMergeAttr> NoMergeAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<NoMergeAttr> NoMergeAttr::from(const VariantEntity &e) {
