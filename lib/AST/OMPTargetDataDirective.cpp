...
std::optional<OMPTargetDataDirective> OMPTargetDataDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPTargetDataDirective, ir::Operation>> OMPTargetDataDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPTargetDataDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTargetDataDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
