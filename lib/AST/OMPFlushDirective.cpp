...
std::optional<OMPFlushDirective> OMPFlushDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPFlushDirective, ir::Operation>> OMPFlushDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPFlushDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPFlushDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
