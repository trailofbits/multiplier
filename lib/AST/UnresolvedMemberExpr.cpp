...
std::optional<UnresolvedMemberExpr> UnresolvedMemberExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<UnresolvedMemberExpr, ir::hl::Operation>> UnresolvedMemberExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kUnresolvedMemberExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<UnresolvedMemberExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
