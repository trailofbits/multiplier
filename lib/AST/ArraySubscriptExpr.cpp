...
std::optional<ArraySubscriptExpr> ArraySubscriptExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ArraySubscriptExpr, ir::hl::Operation>> ArraySubscriptExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kArraySubscriptExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ArraySubscriptExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
