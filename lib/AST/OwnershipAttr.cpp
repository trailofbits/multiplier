...
gap::generator<OwnershipAttr> OwnershipAttr::in(const Index &index) {
  ...
  for (AttrKind k : kOwnershipAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<OwnershipAttr> e = from_base(std::move(eptr))) {
...
gap::generator<OwnershipAttr> OwnershipAttr::in(const File &file) {
...
std::optional<OwnershipAttr> OwnershipAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<OwnershipAttr> OwnershipAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
std::optional<OwnershipAttr> OwnershipAttr::from(const VariantEntity &e) {
