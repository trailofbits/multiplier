...
std::optional<OMPSingleDirective> OMPSingleDirective::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OMPSingleDirective, ir::hl::Operation>> OMPSingleDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOMPSingleDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPSingleDirective, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
