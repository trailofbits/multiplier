...
std::optional<MaterializeTemporaryExpr> MaterializeTemporaryExpr::from(const ir::Operation &op) {
...
gap::generator<std::pair<MaterializeTemporaryExpr, ir::Operation>> MaterializeTemporaryExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kMaterializeTemporaryExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<MaterializeTemporaryExpr, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
StorageDuration MaterializeTemporaryExpr::storage_duration(void) const {
  return static_cast<StorageDuration>(impl->reader.getVal91());
}
