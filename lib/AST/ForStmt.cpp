...
std::optional<ForStmt> ForStmt::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<ForStmt, ir::hl::Operation>> ForStmt::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kForStmtDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<ForStmt, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
