...
std::optional<OffsetOfExpr> OffsetOfExpr::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<OffsetOfExpr, ir::hl::Operation>> OffsetOfExpr::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::hl::Operation> res : Stmt::in(tu, kOffsetOfExprDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OffsetOfExpr, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
...
std::optional<OffsetOfExpr> OffsetOfExpr::from(const std::optional<Stmt> &parent) {
  if (parent) {
...
std::optional<OffsetOfExpr> OffsetOfExpr::from_base(const Stmt &parent) {
  switch (parent.kind()) {
...
gap::generator<OffsetOfExpr> OffsetOfExpr::in(const Index &index) {
  ...
  for (StmtKind k : kOffsetOfExprDerivedKinds) {
    for (StmtImplPtr eptr : ep->StmtsFor(ep, k)) {
      if (std::optional<OffsetOfExpr> e = from_base(std::move(eptr))) {
...
std::optional<OffsetOfExpr> OffsetOfExpr::from(const VariantEntity &e) {
