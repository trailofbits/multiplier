...
std::optional<OMPTargetSimdDirective> OMPTargetSimdDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPTargetSimdDirective, ir::Operation>> OMPTargetSimdDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPTargetSimdDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTargetSimdDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
