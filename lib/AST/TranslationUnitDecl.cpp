...
std::optional<TranslationUnitDecl> TranslationUnitDecl::from(const ir::hl::Operation &op) {
...
gap::generator<std::pair<TranslationUnitDecl, ir::hl::Operation>> TranslationUnitDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::hl::Operation> res : Decl::in(tu, kTranslationUnitDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<TranslationUnitDecl, ir::hl::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
