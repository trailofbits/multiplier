...
std::optional<TranslationUnitDecl> TranslationUnitDecl::from(const ir::Operation &op) {
...
gap::generator<std::pair<TranslationUnitDecl, ir::Operation>> TranslationUnitDecl::in(const Compilation &tu) {
  for (std::pair<Decl, ir::Operation> res : Decl::in(tu, kTranslationUnitDeclDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<TranslationUnitDecl, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
