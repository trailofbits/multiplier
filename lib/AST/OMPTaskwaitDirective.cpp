...
std::optional<OMPTaskwaitDirective> OMPTaskwaitDirective::from(const ir::Operation &op) {
...
gap::generator<std::pair<OMPTaskwaitDirective, ir::Operation>> OMPTaskwaitDirective::in(const Compilation &tu) {
  for (std::pair<Stmt, ir::Operation> res : Stmt::in(tu, kOMPTaskwaitDirectiveDerivedKinds)) {
    if (auto val = from_base(res.first)) {
      co_yield std::pair<OMPTaskwaitDirective, ir::Operation>(std::move(val.value()), std::move(res.second));
    }
  }
}
