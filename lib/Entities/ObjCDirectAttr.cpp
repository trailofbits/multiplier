...
#include <multiplier/Entities/ObjCDirectAttr.h>
...
std::optional<ObjCDirectAttr> ObjCDirectAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ObjCDirectAttr> ObjCDirectAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
    case ObjCDirectAttr::static_kind():
...
gap::generator<ObjCDirectAttr> ObjCDirectAttr::in(const Index &index) {
  ...
  for (AttrKind k : kObjCDirectAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ObjCDirectAttr> e = from_base(std::move(eptr))) {
...
gap::generator<ObjCDirectAttr> ObjCDirectAttr::in(const File &file) {
...
std::optional<ObjCDirectAttr> ObjCDirectAttr::from(const VariantEntity &e) {
