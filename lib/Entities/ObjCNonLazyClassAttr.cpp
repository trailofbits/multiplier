...
#include <multiplier/Entities/ObjCNonLazyClassAttr.h>
...
std::optional<ObjCNonLazyClassAttr> ObjCNonLazyClassAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<ObjCNonLazyClassAttr> ObjCNonLazyClassAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
    case ObjCNonLazyClassAttr::static_kind():
...
gap::generator<ObjCNonLazyClassAttr> ObjCNonLazyClassAttr::in(const Index &index) {
  ...
  for (AttrKind k : kObjCNonLazyClassAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<ObjCNonLazyClassAttr> e = from_base(std::move(eptr))) {
...
gap::generator<ObjCNonLazyClassAttr> ObjCNonLazyClassAttr::in(const File &file) {
...
std::optional<ObjCNonLazyClassAttr> ObjCNonLazyClassAttr::from(const VariantEntity &e) {
