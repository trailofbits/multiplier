...
std::optional<GNUInlineAttr> GNUInlineAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<GNUInlineAttr> GNUInlineAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<GNUInlineAttr> GNUInlineAttr::in(const Index &index) {
  ...
  for (AttrKind k : kGNUInlineAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<GNUInlineAttr> e = from_base(std::move(eptr))) {
...
gap::generator<GNUInlineAttr> GNUInlineAttr::in(const File &file) {
...
std::optional<GNUInlineAttr> GNUInlineAttr::from(const VariantEntity &e) {
