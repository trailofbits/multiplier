...
std::optional<PureAttr> PureAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<PureAttr> PureAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<PureAttr> PureAttr::in(const Index &index) {
  ...
  for (AttrKind k : kPureAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<PureAttr> e = from_base(std::move(eptr))) {
...
gap::generator<PureAttr> PureAttr::in(const File &file) {
...
std::optional<PureAttr> PureAttr::from(const VariantEntity &e) {
