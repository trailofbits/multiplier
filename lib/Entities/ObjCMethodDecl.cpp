...
ObjCMethodDeclImplementationControl ObjCMethodDecl::implementation_control(void) const {
  return static_cast<ObjCMethodDeclImplementationControl>(impl->reader.getVal83());
}

ObjCMethodFamily ObjCMethodDecl::method_family(void) const {
  return static_cast<ObjCMethodFamily>(impl->reader.getVal88());
}

DeclObjCDeclQualifier ObjCMethodDecl::obj_c_decl_qualifier(void) const {
  return static_cast<DeclObjCDeclQualifier>(impl->reader.getVal90());
}
...
TokenRange ObjCMethodDecl::return_type_tokens(void) const {
  return impl->ep->TokenRangeFor(impl->ep, impl->reader.getVal71(), impl->reader.getVal81());
}
...
gap::generator<ParmVarDecl> ObjCMethodDecl::parameters(void) const & {
  ...
  for (auto v : list) {
    ...
    if (auto d54 = ep->DeclFor(ep, v)) {
      if (auto e = ParmVarDecl::from(Decl(std::move(d54)))) {
        co_yield std::move(*e);
      }
    }
  }
...
gap::generator<Token> ObjCMethodDecl::selector_tokens(void) const & {
  ...
  for (auto v : list) {
    EntityId id(v);
    if (auto t55 = ep->TokenFor(ep, tok_reader, v)) {
      co_yield t55;
    }
  }
  co_return;
