...
#include <multiplier/Entities/AbiTagAttr.h>
...
std::optional<AbiTagAttr> AbiTagAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<AbiTagAttr> AbiTagAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
    case AbiTagAttr::static_kind():
...
gap::generator<AbiTagAttr> AbiTagAttr::in(const Index &index) {
  ...
  for (AttrKind k : kAbiTagAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<AbiTagAttr> e = from_base(std::move(eptr))) {
...
std::optional<AbiTagAttr> AbiTagAttr::from(const VariantEntity &e) {
