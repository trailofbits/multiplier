...
std::optional<HotAttr> HotAttr::from(const std::optional<Attr> &parent) {
  if (parent) {
...
std::optional<HotAttr> HotAttr::from_base(const Attr &parent) {
  switch (parent.kind()) {
...
gap::generator<HotAttr> HotAttr::in(const Index &index) {
  ...
  for (AttrKind k : kHotAttrDerivedKinds) {
    for (AttrImplPtr eptr : ep->AttrsFor(ep, k)) {
      if (std::optional<HotAttr> e = from_base(std::move(eptr))) {
...
gap::generator<HotAttr> HotAttr::in(const File &file) {
...
std::optional<HotAttr> HotAttr::from(const VariantEntity &e) {
