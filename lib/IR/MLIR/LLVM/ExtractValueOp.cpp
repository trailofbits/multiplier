...
#include <multiplier/IR/MLIR/LLVM/ExtractValueOp.h>
...
::mlir::LLVM::ExtractValueOp ExtractValueOp::underlying_op(void) const noexcept {
  return ::mlir::LLVM::ExtractValueOp(this->Operation::op_);
}

::mx::ir::Value ExtractValueOp::container(void) const {
  auto val = underlying_op().getContainer();
