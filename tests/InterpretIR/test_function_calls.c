 ...
 * function add (NORMAL) {
 *   objects:
 *     obj_0 PARAMETER_VALUE size=4 align=1 (a)
 *     obj_1 PARAMETER_VALUE size=4 align=1 (b)
 ...
 *     >> %3 = ENTER_SCOPE  // {     return a + b; }
 *          %9 = RETURN_PTR  // return a + b
 *          %8 = ADD [%6, %7]  // a + b
 *     >> %10 = MEMORY/STORE_LE_32 [%9, %8]  // return a + b
 *     >> %11 = EXIT_SCOPE  // {     return a + b; }
 *          %8 = ADD [%6, %7]  // a + b
 *     >> %12 = RET [%8]  // return a + b
 * }
 * function factorial (NORMAL) {
 *   objects:
 *     obj_0 PARAMETER_VALUE size=4 align=1 (n)
 ...
 *     -> [block_1]
 *   block_1 ENTRY  <- [block_0]:
 *     >> %2 = ENTER_SCOPE  // {     if (n <= 1) return 1;     return n * fact...
 *          %6 = CMP_LE [%4, %5]  // n <= 1
 *     >> %7 = COND_BRANCH [%6]  // if (n <= 1) return 1
 *     -> [block_2, block_3]
 *   block_3 IF_ELSE  <- [block_1]:
 *     >> %13 = IMPLICIT_GOTO
 *     -> [block_4]
 *   block_4 IF_MERGE  <- [block_3]:
 *     >> %15 = ENTER_SCOPE  // factorial(n - 1)
 *          %19 = ALLOCA/ARG size=4 align=1  // n - 1
 *          %18 = SUB [%16, %17]  // n - 1
 *     >> %20 = MEMORY/STORE_LE_32 [%19, %18]  // n - 1
 *     >> %24 = EXIT_SCOPE
 *          %25 = RETURN_PTR  // return n * factorial(n - 1)
 *          %23 = MUL [%14, %22]  // n * factorial(n - 1)
 *     >> %26 = MEMORY/STORE_LE_32 [%25, %23]  // return n * factorial(n - 1)
 *     >> %27 = EXIT_SCOPE  // {     if (n <= 1) return 1;     return n * fact...
 *          %23 = MUL [%14, %22]  // n * factorial(n - 1)
 *     >> %28 = RET [%23]  // return n * factorial(n - 1)
 *   block_2 IF_THEN  <- [block_1]:
 *          %9 = RETURN_PTR  // return 1
 *          %8 = CONST/INT32 1  // 1
 *     >> %10 = MEMORY/STORE_LE_32 [%9, %8]  // return 1
 *     >> %11 = EXIT_SCOPE  // {     if (n <= 1) return 1;     return n * fact...
 *          %8 = CONST/INT32 1  // 1
 *     >> %12 = RET [%8]  // return 1
 * }
 * function apply (NORMAL) {
 *   objects:
 *     obj_0 PARAMETER_VALUE size=8 align=1 (fn)
 *     obj_1 PARAMETER_VALUE size=4 align=1 (x)
 *     obj_2 PARAMETER_VALUE size=4 align=1 (y)
 *     obj_3 RETURN_SLOT size=4 align=1
 ...
 *   blocks:
 *   block_0 FRAME:
 *     >> %fn.0 = ALLOCA/LOCAL size=8 align=1
 *     >> %x.1 = ALLOCA/LOCAL size=4 align=1
 *     >> %y.2 = ALLOCA/LOCAL size=4 align=1
 *     >> %3 = IMPLICIT_GOTO
 *     -> [block_1]
 *   block_1 ENTRY  <- [block_0]:
 *     >> %4 = ENTER_SCOPE  // {     return fn(x, y); }
 *     >> %8 = ENTER_SCOPE  // fn(x, y)
 *          %11 = ALLOCA/ARG size=4 align=1  // x
 *          %10 = MEMORY/LOAD_LE_32 [%x.6]  // x
 *     >> %12 = MEMORY/STORE_LE_32 [%11, %10]  // x
 *          %14 = ALLOCA/ARG size=4 align=1  // y
 *          %13 = MEMORY/LOAD_LE_32 [%y.7]  // y
 *     >> %15 = MEMORY/STORE_LE_32 [%14, %13]  // y
 *     >> %18 = EXIT_SCOPE
 *          %19 = RETURN_PTR  // return fn(x, y)
 *          %17 = CALL indirect [%9, %11, %14]  // fn(x, y)
 *     >> %20 = MEMORY/STORE_LE_32 [%19, %17]  // return fn(x, y)
 *     >> %21 = EXIT_SCOPE  // {     return fn(x, y); }
 *          %17 = CALL indirect [%9, %11, %14]  // fn(x, y)
 *     >> %22 = RET [%17]  // return fn(x, y)
 * }
 ...
 *          %9 = CONST/INT32 0  // 0
 *     >> %sum.10 = MEMORY/STORE_LE_32 [%sum.2, %9]
 *     >> %11 = ENTER_SCOPE  // for (int i = 0; i < count; i++) {         sum +...
 *     >> %12 = IMPLICIT_GOTO
 *     -> [block_2]
 ...
 *     >> %19 = COND_BRANCH [%18]  // for (int i = 0; i < count; i++) {         sum +...
 *     -> [block_4, block_6]
 *   block_6 LOOP_EXIT  <- [block_3]:
 *     >> %29 = EXIT_SCOPE  // for (int i = 0; i < count; i++) {         sum +...
 *          %30 = MEMORY/LOAD_LE_64 [%ap.1]  // ap
 *     >> %31 = VA_END [%30]  // __builtin_va_end(ap)
 *          %33 = RETURN_PTR  // return sum
 *          %32 = MEMORY/LOAD_LE_32 [%sum.2]  // sum
 *     >> %34 = MEMORY/STORE_LE_32 [%33, %32]  // return sum
 *     >> %35 = EXIT_SCOPE  // {     va_list ap;     __builtin_va_start(ap,cou...
 *          %32 = MEMORY/LOAD_LE_32 [%sum.2]  // sum
 *     >> %36 = RET [%32]  // return sum
 *   block_4 LOOP_BODY  <- [block_3]:
 *     >> %20 = ENTER_SCOPE  // {         sum += __builtin_va_arg(ap,int);     }
 *          %sum.2 = ALLOCA/LOCAL size=4 align=1
 *          %22 = MEMORY/CONSUME_VA_PARAM [%21]  // __builtin_va_arg(ap,int)
 *     >> %23 = READ_MODIFY_WRITE(ADD new) [%sum.2, %22]  // sum += __builtin_va_arg(ap,int)
 *     >> %24 = EXIT_SCOPE  // {         sum += __builtin_va_arg(ap,int);     }
 *     >> %25 = IMPLICIT_GOTO
 *     -> [block_5]
