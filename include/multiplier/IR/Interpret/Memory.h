...
class Memory {
 public:
  ...
  virtual bool write_pointer(uint64_t address, uint64_t pointer_value);
  virtual bool read_pointer(uint64_t address, uint64_t &pointer_value);
  ...
  virtual bool is_poisoned(uint64_t address) const {
    (void) address;
    return false;
  }
...
// Default implementations. write_pointer writes raw bytes; read_pointer returns
// false (no pointer tracking). Override both in implementations that track
// pointer provenance (e.g., ConcreteMemory).
inline bool Memory::write_pointer(uint64_t address, uint64_t pointer_value) {
  return write(address, &pointer_value, sizeof(pointer_value));
}

inline bool Memory::read_pointer(uint64_t address, uint64_t &pointer_value) {
  (void) address;
  (void) pointer_value;
  return false;
}
