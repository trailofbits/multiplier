...
class CXXParenListInitExpr : public Expr {
 ...
 public:
  ...
  static std::optional<CXXParenListInitExpr> from(const Stmt &parent);

  inline static std::optional<CXXParenListInitExpr> from(const std::optional<Stmt> &parent) {
    if (parent) {
      return CXXParenListInitExpr::from(parent.value());
    } else {
      return std::nullopt;
    }
  }
