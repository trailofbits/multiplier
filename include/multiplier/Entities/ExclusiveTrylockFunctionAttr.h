...
class ExclusiveTrylockFunctionAttr : public InheritableAttr {
 ...
 public:
  ...
  std::optional<ExclusiveTrylockFunctionAttr> by(const Index &, EntityId);

  inline static constexpr AttrKind static_kind(void) {
    return AttrKind::EXCLUSIVE_TRYLOCK_FUNCTION;
  }

  inline static std::optional<ExclusiveTrylockFunctionAttr> from(const Reference &r) {
    return from(r.as_attribute());
  }

  inline static std::optional<ExclusiveTrylockFunctionAttr> from(const TokenContext &t) {
