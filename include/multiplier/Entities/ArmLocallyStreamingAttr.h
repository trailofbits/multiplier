...
class ArmLocallyStreamingAttr : public InheritableAttr {
 ...
 public:
  ...
  static std::optional<ArmLocallyStreamingAttr> from(const Attr &parent);

  inline static std::optional<ArmLocallyStreamingAttr> from(const std::optional<Attr> &parent) {
    if (parent) {
      return ArmLocallyStreamingAttr::from(parent.value());
    } else {
      return std::nullopt;
    }
  }
